        ...
        if os.path.isdir(server_path):
            ...
            for data_file in os.listdir(iteration_path):
                if not os.path.isdir(data_file) and data_file.startswith("tick_log"):
                    ...
                    traces.append(go.Scatter(x=iteration_df["timestamp"],y=iteration_df["rolling"], line=line_type, xaxis=curr_xaxis, yaxis = 'y2',name=host, showlegend=show_legend, line_shape="spline"))
                    show_legend = False
            server_num += 1
            traces.append(go.Scatter(x=[0,300],y=[50,50], mode='lines', showlegend=False,xaxis=curr_xaxis, yaxis='y2', line=dict(color='black', dash='dash',width=2),name='Overloaded Point'))    
    host_num+=1
...
for host in hosts:
    curr_root = root.joinpath(f"results_{host}_runaway")
    show_legend = True
    server_num = 0 
        ...
        if os.path.isdir(server_path):
            ...
            for data_file in os.listdir(iteration_path):
                if not os.path.isdir(data_file) and data_file.startswith("tick_log"):
                    ...
                    traces.append(go.Scatter(x=iteration_df["timestamp"],y=iteration_df["rolling"], line=line_type, xaxis=curr_xaxis, yaxis = 'y',name=host, showlegend=False, line_shape="spline"))
                    show_legend = False
            server_num += 1 
            traces.append(go.Scatter(x=[0,300],y=[50,50], mode='lines', showlegend=False,xaxis=curr_xaxis, yaxis='y', line=dict(color='black', dash='dash',width=2),name='Overloaded Point'))
    host_num+=1

traces.append(go.Scatter(x=[0,300],y=[50,50], mode='lines', showlegend=True,xaxis=curr_xaxis, yaxis='y2', line=dict(color='black', dash='dash',width=2),name='Overloaded Point'))    
...
with open(str(output_dir.joinpath("multi_baseline_and_runaway.pdf")), "wb") as f:
