...
df = pandas.DataFrame(columns = ["server","iteration","timestamp","mem"])
fig_line = go.Figure()
servers = []
server_num = 0
        ...
        for data_file in os.listdir(iteration_dir):
            if not os.path.isdir(data_file) and data_file.startswith("sys_metrics"):
                ...
                if server_num == 1:
                    ...
                    symbol = "square"
                server_num +=1
                fig_line.add_trace(go.Scatter(x=iteration_df["timestamp"],y=iteration_df["proc.memory_full_info.uss"], line=line_type, name=current_server, line_shape="spline"))
...
fig_line.update_layout(legend=dict(y=1,font_size=16), xaxis_title="Time [s]", yaxis_title="Memory (USS) [GB]", font=dict(size=16))


with open(str(output_dir.joinpath("mem_line.pdf")), "wb") as f:
...
for server in servers:
    ...
    if server_num == 1:
        ...
        fig_violin.add_trace(go.Violin(x=df['server'][df['server'] == server],
                            y=df["proc.memory_full_info.rss"][df['server'] == server],
                            name=server,
                            box_visible=True,
...
fig_violin.update_layout(yaxis_title="Memory (RSS) [GB]", font=dict(size=16))


with open(str(output_dir.joinpath("mem_violin.pdf")), "wb") as f:
