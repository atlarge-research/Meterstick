        ...
        for data_file in os.listdir(iteration_dir):
            if not os.path.isdir(data_file) and data_file.startswith("sys_metrics"):
                ...
                if server_num == 1:
                    ...
                    symbol = "square"
                server_num +=1
                fig_line.add_trace(go.Scatter(x=iteration_df["timestamp"],y=iteration_df["proc.io_counters.write_bytes"], line=line_type, name=current_server, line_shape="spline"))
...
fig_line.update_layout(legend=dict(y=1,font_size=16), xaxis_title="Time [s]", yaxis_title="Disk Write [B]", font=dict(size=16))


with open(str(output_dir.joinpath("disk_line.pdf")), "wb") as f:
